var GlobalConfig = {
    ...
    msg_type_toastr: "toastr", msg_type_layer: "layer",
}
...
(function ($) {
    $.extend({
        layer: {
            msg: function (content, type, options) {
                ...
                var opts = $.extend({
                    skin: layer_skin, shade: [0.1, '#000'], shadeClose: true
            ...
            confirm: function (content, callbackOK, callbackCancel) {
                var options = {
                    btn: ['确定', '取消'], icon: 3, title: '询问', shade: [0.3, '#000']
                };
            ...
            closeLoading: function () {
                ...
                layer.tips(msg, attachment, {
                    tips: [position, '#3595CC'], time: 5000
                });
            }
    ...
    var Fast = {
        confirm: function (title, okFunction, cancelFunction) {
            ...
            $.ajax({
                async: false, type: "GET", url: url, error: function (data, status) {
                    //这里需要覆盖ajaxSetup中的方法
...
(function ($) {
    $.fn.toTop = function (opt) {
        ...
        if (!options) {
            options = $.extend({
                autohide: true, offset: 100, speed: 500, right: 15, bottom: 50
            }, opt);
        }
        elem.css({
            'position': 'fixed', 'right': options.right, 'bottom': options.bottom, 'cursor': 'pointer'
        });
        if (options.autohide) {
